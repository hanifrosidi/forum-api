...
describe("commentLikes repository postgres test", () => {
  ...
  describe("addLike function test", () => {
    ...
    it("should pass if comment and owner reference valid", async () => {
      // Arrange
      ...
      // Assert
      await expect(
        commentLikeRepository.addLike(like),
  ...
  describe("deleteLike function test", () => {
    it("should pass delete if delete like success", async () => {
      // Arrange
      const commentId = `comment-${nanoid()}`;
      const userId = `user-${nanoid()}`;
      const threadId = `thread-${nanoid()}`;
      ...
      // Assert
      await expect(
        commentLikeRepository.deleteLike(like),
      ).resolves.not.toThrowError();
    });
  });
