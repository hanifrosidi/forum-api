...
describe("CommentRepository postgres test", () => {
  ...
  describe("addComment function test", () => {
    ...
    it("should return comment data if payload is invalid", async () => {
      // Arrange
      const userId = `user-${Date.now()}`;
    ...
    it("should return comment data if payload is valid", async () => {
      // Arrange
      const userId = `user-${Date.now()}`;
      const threadId = `thread-${Date.now()}`;
      const date = new Date().toISOString();
      ...
      await expect(
        commentRepository.addComment(userId, threadId, commentPayload),
      ).resolves.toBeInstanceOf(AddedComment);
    });
  });
  ...
  describe("checkCommentAvailability", () => {
    ...
    it("sshould throw error if comment delete", async () => {
      // Arrange
      const userId = `user-${Date.now()}`;
      const threadId = `thread-${Date.now()}`;
      const commentId = `comment-${Date.now()}`;
